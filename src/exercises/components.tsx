...
const ExerciseComponent: React.FC<ExerciseProps> = (props: ExerciseProps) => {
  ...
  let answerColor = _.isEqual(props.expectedResult, props.result)
    ? rightAnswer
    : wrongAnswer;
  return (
    <DrawerAppContent>
...
export const MapStatusCode: React.FC = () => {
  ...
  useEffect(() => {
    puzzles.mapStatus("/invalidUrl").subscribe(code => setStatusCode(code));
  }, []);
...
export const TapToConsoleLog: React.FC = () => {
  ...
  const [state, updateState] = useState<TappedInfo>({
    logged: false,
    nums: []
  });
  ...
  useEffect(() => {
    const phonyLogger = {
      log(...args: any[]) {
        updateState(state => {
          return { logged: true, nums: state.nums };
        });
        console.log(args);
      },
      error(...args: any[]) {}
    ...
    puzzles.logging(phonyLogger, of(1, 2, 3)).subscribe(num =>
      updateState(state => {
        return {
          logged: state.logged,
          nums: [...state.nums, num]
...
export const GetTheJSON: React.FC = () => {
  ...
  useEffect(() => {
    puzzles.getTheJSON("/posts").subscribe(body => setResponseBody(body));
  }, []);
...
export const TakeFiveRowsFromTheTweets: React.FC = () => {
  ...
  useEffect(() => {
    puzzles.takeFiveRows("/nintendo").subscribe(row => {
...
export const CountValidUsers: React.FC = () => {
  ...
  useEffect(() => {
    puzzles.countValidUsers("/nintendo").subscribe(count => setCount(count));
  }, []);
...
export const CatchErrorEmitsASuccessMessage: React.FC = () => {
  ...
  useEffect(() => {
    const observableWithError = concat(
      of("Right Url"),
      throwError("Wrong Url")
    );

    puzzles
      .catchErrorEmitsASuccessMessage(observableWithError)
...
export const ConvertSuccessIntoErrorWithThrow: React.FC = () => {
  ...
  useEffect(() => {
    puzzles.convertSuccessfulFetchIntoError("/badUrl").subscribe(
      () => {},
...
export const ChainFetches: React.FC = () => {
  ...
  const expectedResult = [
    ...
    [
      "jojade74",
      "@Johnny_G86 @bbcnickrobinson he was a young Tory -comes from Cheadle Hulme"
    ]
  ];
...
export const CreateYourOwnFetch: React.FC = () => {
  ...
  useEffect(() => {
    ...
    puzzles
      .createYourOwnFetch(goodRequestUrl)
      .pipe(
      ...
      .subscribe(
        msg => setMessages(messages => [...messages, msg]),
...
export const CompleteASubscription: React.FC = () => {
  ...
  useEffect(() => {
    puzzles.completeASubscription().subscribe(
      val => setMessages(m => [...m, val]),
